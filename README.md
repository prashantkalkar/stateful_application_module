...
| <a name="input_node_config_script"></a> [node\_config\_script](#input\_node\_config\_script) | Base64 encoded node configuration shell script.<br/>  Must include configure\_cluster\_node and wait\_for\_healthy\_cluster function. Check documentation for more details about the contract.<br/>  Note: If script exceeds AWS userdata 16KB limit, use node\_config\_script\_s3\_url instead. | `string` | `null` | no |
| <a name="input_node_config_script_s3_url"></a> [node\_config\_script\_s3\_url](#input\_node\_config\_script\_s3\_url) | S3 URL for the node configuration shell script (e.g., s3://bucket-name/path/to/script.sh).<br/>  Used to download the script when it exceeds AWS userdata 16KB limit.<br/>  Must include configure\_cluster\_node and wait\_for\_healthy\_cluster function. Check documentation for more details about the contract.<br/>  Note: This takes precedence over node\_config\_script if both are provided. At least one must be provided. | `string` | `null` | no |
